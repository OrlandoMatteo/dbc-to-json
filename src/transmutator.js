...
const parseDbc = (dbcString, options = {}) => {
	//console.debug(`The raw dbcString:\n`, dbcString)

	let dbcArray = dbcString.split("\n")

	//console.debug(`dbcArray:\n`, dbcArray)
	...
	_.each(dbcData, (line, index) => {
		...
		switch (line[0]) {
			...
			case ("CM_"): // CM_ [<BU_|BO_|SG_> [CAN-ID] [SignalName]] "<DescriptionText>";
				...
				break
			case ("BA_"):
				if (line[1].includes("CI_SigId")) {
					let { sigBoLink, signalName, signalId } = extractSignalId(line)
					signalIdList.push({ sigBoLink, signalName, signalId })
				}
				break
	...
	//console.debug(signalIdList)
	signalIdList.forEach((signal) => {
		let bo = _.find(boList, { canId: signal.sigBoLink })
		if (!bo) {
			return
		}
		let sg = _.find(bo.signals, { name: signal.signalName })
		if (!sg) {
			return
		}
		sg.signalId = signal.signalId
	})

	let result = { "params": boList, "problems": problems }
	//console.debug(JSON.stringify(boList, null, 4))
